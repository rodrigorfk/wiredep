...
function injectScripts(filePath) {
  filePath = $.path.join(config.get('cwd'), filePath);
  var contents = $.fs.readFileSync(filePath, {encoding:'utf8'});
  var iconv = new Iconv(config.get('encoding'), 'UTF-8');
  ...
  var newContents = contents.replace(
    ...
    if (contents !== newContents) {
        iconv = new Iconv('UTF-8', config.get('encoding'));
        newContents = iconv.convert(newContents);
      
        $.fs.writeFileSync(filePath, newContents.toString('utf8'));
