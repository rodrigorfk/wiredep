...
function gatherInfo(config) {
  ...
  return function (version, component) {
    ...
    var self = config.get('bower.json');

    var componentConfigFile = findComponentConfigFile(config, component);
    if (!componentConfigFile) {
        if (overrides && overrides[component]) {
            componentConfigFile = overrides[component];
        }
        if (!componentConfigFile) {
            if (self.overrides && self.overrides[component]) {
                componentConfigFile = self.overrides[component];
            }
            if (!componentConfigFile) {
                var configFile = $.path.join(config.get('bower-directory'), component);
                var error = new Error(component + ' is not installed in ' + configFile + '. Try running `bower install`.');
